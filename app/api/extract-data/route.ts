...
Para o campo "servico", extrai o serviço ou categoria da PCI-TechLab que melhor corresponde à necessidade real da empresa/projeto, com base no problema, objetivo, contexto e materiais discutidos.
Não uses "Consultoria/Assessoria" ou "Brainstorming/Ideacao" - extrai o serviço técnico real (ex: "Impressão 3D", "Aplicações Web", "Realidade Aumentada/Virtual", "Motion Capture", etc.)
Se a necessidade for exploração de ideia sem um serviço técnico definido, indica o serviço mais próximo do que foi discutido.

Retorna um JSON válido (apenas JSON, sem markdown) com a seguinte estrutura:
{
  "empresa": "nome da empresa/cliente do utilizador",
  "servico": "tipo de serviço técnico específico (ex: Impressão 3D, Aplicações Web, Conteúdos Imersivos AR/VR, Motion Capture, etc.)",
...
For the "servico" field, extract the PCI-TechLab service or category that best matches the company's real need, based on the problem, goal, context, and materials discussed.
Do not use "Consultoria/Assessoria" or "Brainstorming/Ideacao" - extract the actual technical service (ex: "3D Printing", "Web Applications", "AR/VR Immersive Content", "Motion Capture", etc.)
If the need is idea exploration without a defined technical service, indicate the service closest to what was discussed.

Return valid JSON (only JSON, no markdown) with the following structure:
{
  "empresa": "name of the client's company/organization",
  "servico": "specific technical service type (ex: 3D Printing, Web Applications, AR/VR Immersive Content, Motion Capture, etc.)",
